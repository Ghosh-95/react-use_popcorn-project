...
const ratingStarContainerStyle = {
    display: "flex",
};
...
export default function StarRating({ maxRating = 5, color = "#fcc419", size = 48, messages = [], defaultRating = 0, onSetRating }) {
    ...
    return (
        <article style={ratingContainerStyle}>
            <div style={ratingStarContainerStyle}>
                {Array.from({ length: Number(maxRating) }, (_, i) => (
                    <Star key={i}
                        onRate={() => handleRating(i + 1)}
                        full={tempRating ? tempRating >= i + 1 : rating >= i + 1}
                        onHover={() => setTempRating(i + 1)} onLeave={() => setTempRating(0)}
...
function Star({ onRate, full, onHover, onLeave, color, size }) {
    ...
    return (
        <span role="button"
            style={{ ...starStyles, height: size, width: size }}
            onClick={onRate}
